...
class SupervisedTrainer(object):
    ...
    def train_epoch(self, train_db, optimizer, epoch):
		...
		#added - Simon Walker
		tr_writer = SummaryWriter(log_dir=os.path.join(self.log_dir, "train"))
        va_writer = SummaryWriter(log_dir=os.path.join(self.log_dir, "valid"))
		#iter_idx = -1
		#added - Simon Walker
        ...
        for cnt, batched in enumerate(train_loader):
			...
			#added - Simon Walker
			acc = np.mean(np.array(train_accu).float(), 0) * 100.0
    ...
    def validate_epoch(self, val_db):
        ...
        for G in range(2):
			...
			#added - Simon Walker
			va_loss = np.mean(val_loss)
            va_acc = np.mean(val_accu)
