...
def train(config):
    ...
    tr_data_loader = DataLoader(train_data)
    va_data_loader = DataLoader(valid_data)
    ...
    if os.path.exists(checkpoint_file):
        if config.resume:
            ...
            file = torch.load(checkpoint_file)

            mod = file['model']
            opt = file['optimizer']
            iter_idx = file['iter_idx']
            best_va_acc = file['best_va_acc']
        ...
        for data in tqdm(tr_data_loader, desc=prefix):
            ...
            if iter_idx % config.rep_intv == 0:
                ...
                for sample in tr_data_loader:
                    ...
                    scores = model.forward(x)
                    te_loss += [data_loss(scores, y)]
                    correct = (torch.argmax(scores, dim=1))
                    te_acc += [correct.float()]
                    
                # TODO (2 points): Write loss and accuracy to tensorboard,
                # using keywords `loss` and `accuracy`.
                tr_writer.add_scalar('rep_loss', np.mean(rep_loss))
                tr_writer.add_scalar('rep_accuracy', np.mean(rep_acc))
            ...
            if iter_idx % config.val_intv == 0:
                ...
                va_writer.add_scalar('va_loss', va_loss)
                va_writer.add_scalar('va_accuracy', va_acc)
...
def test(config):
    ...
    # shuffling.
    te_data_loader = DataLoader(test_data, batch_size=config.batch_size)

    # Create model
    model = MyNetwork(config, test_data.sample_shp)
    ...
    best_model = os.path.join(config.save_dir, "best_model.pth")
