...
class MyNetwork(nn.Module):
    ...
    def __init__(self, config, input_shp, mean=None, std=None):
        ...
        if mean is None:
            m = torch.zeros(input_shp, dtype=torch.float32)
            m = torch.tensor(m)
        else:
            m = torch.tensor(mean, dtype=torch.float32)

        if std is None:
            s = torch.ones(input_shp, dtype=torch.float32)
            s = torch.tensor(s)
        else:
        ...
        self.mean = nn.Parameter(m, requires_grad=False)
        self.std = nn.Parameter(s, requires_grad=False)
        ...
        self.input_layer = nn.Linear(input_shp[0], config.num_unit)
        for i in range(self.config.num_hidden):
            setattr(self, "layer_{}".format(i), nn.Linear(config.num_unit, config.num_unit))
        #        
        
		
        indim = config.num_hidden
    ...
    def forward(self, x):
        ...
        for i in range(self.num_hidden):
            #outdim = getattr(self, "layer_{}".format(i))
            #x = nn.functional.relu(layer)
