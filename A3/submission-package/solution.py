...
# V00787710
# References:
# https://stackoverflow.com/questions/37370369/numpy-how-can-i-select-specific-indexes-in-an-np-array-for-k-fold-cross-validat/41336592#41336592
# https://machinelearningmastery.com/implement-logistic-regression-stochastic-gradient-descent-scratch-python/
# https://www.analyticsvidhya.com/blog/2018/05/improve-model-performance-cross-validation-in-python-r/
# Lecture slides given by Kwang Moo Yi
...
def train(x_tr, y_tr, x_va, y_va, config):
    ...
    W = 0.0005 * np.random.rand(len(x_tr[0]), num_class)
    ...
    for idx_epoch in range(num_epoch):
        ...
        perm_tr = np.random.permutation(len(x_tr_n))
        x = np.array(x_tr_n)
        y = np.array(y_tr)
        x_tr_n = np.asarray(x[perm_tr].tolist())
        y_tr = np.asarray(y[perm_tr].tolist())

        order_of_batches = np.arange(num_batch)
        np.random.shuffle(order_of_batches)

        bx = np.split(x_tr_n, num_batch)
        by = np.split(y_tr, num_batch)
        ...
        for idx_batch in range(num_batch):
            ...
            x_b = bx[order_of_batches[idx_batch]]
            y_b = by[order_of_batches[idx_batch]]
            ...
            W = W + ((-1) * config.learning_rate * dW)
            b = b + ((-1) * config.learning_rate * db)
        ...
        pred = predict(W,b,x_va_n,config) #function implemented above
        acc = np.mean(pred == y_va)
    ...
    train_res = {"W_best":W_best, "b_best": b_best, "best_acc": best_acc, "x_tr_mean":x_tr_mean,"x_tr_range":x_tr_range}
...
def main(config):
    ...
    print("Randomly shuffling data and labels...")
    perm_tr = np.random.permutation(len(x_trva))
    x_trva = np.array(x_trva)[perm_tr]
    y_trva = np.array(y_trva)[perm_tr]

    perm_te = np.random.permutation(len(x_te))
    x_te = np.array(x_te)[perm_te]
    y_te = np.array(y_te)[perm_te]
    ...
    for idx_va_fold in va_fold_to_test:
        ...
        x_tr = x_trva[idx_va_fold][1000: , :]
        x_va = x_trva[idx_va_fold][1000: , :]
        y_tr = y_trva[idx_va_fold][1000:]
        y_va = y_trva[idx_va_fold][1000:]
    ...
    if config.cross_validate:
        ...
        best_acc = [result["best_acc"] for result in train_res]
        val_acc = np.mean(best_acc, axis=0)
        ...
        norm,_,_ = normalize(x_va, x_tr_mean, x_tr_range)
        pred = predict(W,b,norm,config)
